...
class Tour:
    ...
    def get_length(self) -> float:
        return sum([track.get_length() for track in self.tracks])

    def middle_point(self):
        middle_track = self.tracks[len(self.tracks) // 2]
        return middle_track.middle_point()


def tour_from_folder(folder: str) -> Tour:
...
def find_tours():
    for tour_folder in os.listdir(tours_folder):
        tour_folder = os.path.join(tours_folder, tour_folder)
    if os.path.isfile(tour_folder):
        continue
    tour = tour_from_folder(tour_folder)
...
def make_folium_map(gpx_files,
    ...
    for file_name in gpx_files:
        ...
        if activity=='cycling':
            ...
            activity_color=['empty','green','blue','red','orange','darkgreen','purple','blue','darkred','red','orange','cadetblue','green','blue','red','orange','green']
